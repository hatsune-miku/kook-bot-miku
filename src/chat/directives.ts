...
import { KEvent, KEventType, KTextChannelExtra, KUser } from '../websocket/kwebsocket/types'

export class ChatDirectivesManager implements IChatDirectivesManager {
  private guildIdToUserIdToProperties = new Map<string, Map<string, UserProperties>>()
  private contextManager: ContextManager | null = null

  constructor(private eventEmitter: EventEmitter) {}

  respondToUser(params: RespondToUserParameters): Promise<KResponseExt<CreateChannelMessageResult>> {
    return new Promise((resolve) => {
      this.eventEmitter.emit(Events.RespondToUser, params, resolve)
    })
  }

  respondCardMessageToUser(params: RespondToUserParameters): Promise<KResponseExt<CreateChannelMessageResult>> {
    return new Promise((resolve) => {
  ...
  async handleAssignRole(event: ParseEventResultValid) {
    ...
    mentionedUsers.forEach((user) => {
      ConfigUtils.updateGuildConfig(event.originalEvent.extra.guild_id, (config) => {
        if (!config) {
          return config
        }
        config.users ||= {}
  ...
  async handleQuery(event: ParseEventResultValid) {
    ...
    const user = await this.getUser(mayBeTargetUserId ?? userIds[0], event.originalEvent.extra.guild_id)
  ...
  async handlePrintContext(event: ParseEventResultValid) {
    ...
    this.respondToUser({
      ...
      content: mixedContext
        .map(
          (unit) =>
            `${unit.name} (${unit.id}): ${unit.content.length > 32 ? unit.content.slice(0, 32) + '...' : unit.content}`
        )
        .join('\n'),
    })
  }
  ...
  dispatchDirectives(parsedEvent: ParseEventResultValid, onContextReady?: () => void): boolean {
...
function prepareBuiltinDirectives(manager: ChatDirectivesManager): ChatDirectiveItem[] {
  return [
    ...
    },
    {
      ...
      description: `更换当前频道 AI 实现，可选范围：${Object.values(ChatBotBackend).join(', ')}`,
      defaultValue: undefined,
      permissionGroups: ['admin'],
    ...
    {
      triggerWord: 'roll',
      parameterDescription: 'prize-name=<prize-name> prize-count=<prize-count> until=YYYYMMDDHHMMSS',
      description: '发起抽奖',
      defaultValue: undefined,
      permissionGroups: ['admin'],
    ...
    {
      triggerWord: 'vote',
      parameterDescription: 'title=<title> until=YYYYMMDDHHMMSS options=<option1>[,<option2>,...]',
      description: '发起投票',
      defaultValue: undefined,
      permissionGroups: ['admin'],
